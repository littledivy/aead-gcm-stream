    ...
    Aes: BlockCipher + BlockSizeUser<BlockSize = U16> + BlockEncrypt + KeyInit,
{
    ...
    pub fn encrypt(&mut self, block: &mut GenericArray<u8, U16>) {
...
mod tests {
    ...
    fn test_encrypt() {
        for vector in TEST_VECTORS {
            ...
            if vector.plaintext.len() == 16 {
                let mut p = GenericArray::from_slice(vector.plaintext).clone();
                cipher.encrypt(&mut p);
                let tag = cipher.finish();

                assert_eq!(vector.ciphertext, p.as_slice());
    ...
    fn test_decrypt() {
        for vector in TEST_VECTORS {
            ...
            if ciphertext.len() == 16 {
                let mut p = GenericArray::from_slice(&ciphertext).clone();
                cipher.decrypt(&mut p);
                let tag = cipher.finish();

                assert_eq!(vector.plaintext, p.as_slice());
                assert_eq!(vector.tag, tag.as_slice());
            }
        }
    }
    ...
    fn encrypt_1_data_block() {
        ...
        let mut data = GenericArray::from(PT);
    ...
    fn decrypt_1_data_block() {
        ...
        let mut data = GenericArray::from(CT);
        let mut cipher = AesGcm::<Aes128>::new(&KEY.into());
